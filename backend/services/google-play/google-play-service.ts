  ...
  static async getApps(options: {
    ...
    try {
      ...
      const formatted: GooglePlayAppItem[] = (results || []).map((app: any) => ({
        ...
        icon: app.icon || 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&w=800&q=80',
        developer: app.developer,
        developerId: app.developerId,
        priceText: app.priceText || (app.free ? 'Free' : `$${app.price || '0.99'}`),
        free: app.free ?? true,
        score: app.score,
  ...
  static async getAppDetails(
    ...
    try {
      ...
      const item: GooglePlayAppItem = {
        ...
        developer: app.developer,
        developerId: app.developerId,
        priceText: app.priceText || (app.free ? 'Free' : `$${app.price}`),
        free: app.free,
        score: app.score,
        scoreText: app.scoreText,
        summary: app.summary,
